...
def checkout():
    if request.method == 'POST':
        ...
        if action == 'menu':
            ...
            return redirect(url_for('save_order'))  # Redirect to save_order
        else:
            flash("Invalid action. Please try again.", "error")  # Provide feedback for invalid actions
            return redirect(url_for('step1'))  # Redirect to a safe route
    else:
        if 'random_food_for_days' in session:  # Check for key existence
...
def save_order():
    try:
        ...
        print("Order details to save:", order_details)  # Debugging output
        ...
        print(f"Error saving order: {e}")  # More detailed error message
